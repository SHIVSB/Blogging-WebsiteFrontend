...
function Post(){
    ...
    useEffect(() => {
		...
		if (window.location.search) {
			...
			data.categories = window.location.search.split("=")[1];
		}
    ...
    const fetchPosts = (data) => {
		axios.post(BASE_URL + '/api/v1/post/category', data)
    ...
    const addPost = (event) =>{
        const tosend = document.getElementById("content").value;
        ...
        axios.post(BASE_URL + '/api/v1/post/add', data)
			.then(function (response) {
				if (response.data.success) {
                    return;
				}
    ...
    return (
        <div className="">
            ...
            <div className="text-center">
                <div className={"p-2"}>
                    <input id="content" className={"border-gray-500 border-2 text-center h-24 w-3/4  rounded-lg "} type={"text"} placeholder="Enter your post"/>
                </div>
                <button onClick={addPost} id={"1"} className={"bg-slate-800 p-2 text-white ml-12 mx-2 rounded-lg "}>Add Post to Technology</button>
                <button onClick={addPost} id={"2"} className={"bg-slate-800 p-2 text-white ml-12 mx-2 rounded-lg "}>Add Post to Sports</button>
                <button onClick={addPost} id={"3"} className={"bg-slate-800 p-2 text-white ml-12 mx-2 rounded-lg "}>Add Post to Politics</button>
                <button onClick={addPost} id={"4"} className={"bg-slate-800 p-2 text-white ml-12 mx-2 rounded-lg "}>Add Post to Environment</button>
                <button onClick={addPost} id={"5"} className={"bg-slate-800 p-2 text-white ml-12 mx-2 rounded-lg "}>Add Post to Vehicle</button>
